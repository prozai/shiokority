...
class FraudDetectionTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.envConfig = config['development']
    ...
    #         "admin1@example.com",  # Use a valid admin email
    ...
    #     admin_email = "admin1@example.com"  # Use a valid admin email
    ...
    def _create_test_transaction(self):
        ...
        connection = pymysql.connect(
            host= self.envConfig.MYSQL_HOST,
            user= self.envConfig.MYSQL_USER,
            password= self.envConfig.MYSQL_PASSWORD,
            database= self.envConfig.SHIOKORITY_API_SCHEMA,
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute(sql, (
                    ...
                    'payment',
                    self.timestamp,
                    self.timestamp,  # Using same timestamp for update_on
                    self.test_user_id,
                    '53339185K'
                ))
            connection.commit()
    ...
    def _reset_transaction(self, user_id):
        ...
        try:
            with connection.cursor() as cursor:
                sql = """
                DELETE FROM Transaction WHERE cust_id = %s
                """
                cursor.execute(sql, (user_id,))
            connection.commit()
        except Exception as e:
            print(f"Error resetting test transactions: {e}")
            connection.rollback()
        finally:
            connection.close()
